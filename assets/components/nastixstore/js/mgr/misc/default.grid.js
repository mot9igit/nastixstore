nastixstore.grid.Default = function (config) {
    config = config || {};
    ...
    Ext.applyIf(config, {
        ...
        baseParams: {},
        cls: config['cls'] || 'main-wrapper nastixstore-grid',
        autoHeight: true,
        paging: true,
        remoteSort: true,
        ...
        viewConfig: {
            ...
            getRowClass: function (rec) {
                ...
                if (rec.data['published'] != undefined && rec.data['published'] == 0) {
                    cls.push('nastixstore-row-unpublished');
                }
                if (rec.data['active'] != undefined && rec.data['active'] == 0) {
                    cls.push('nastixstore-row-inactive');
                }
                if (rec.data['deleted'] != undefined && rec.data['deleted'] == 1) {
                    cls.push('nastixstore-row-deleted');
                }
                if (rec.data['required'] != undefined && rec.data['required'] == 1) {
                    cls.push('nastixstore-row-required');
                }
                return cls.join(' ');
            }
        },
    });
    nastixstore.grid.Default.superclass.constructor.call(this, config);
...
Ext.extend(nastixstore.grid.Default, MODx.grid.Grid, {
    ...
    getMenu: function (grid, rowIndex) {
        ...
        var menu = nastixstore.utils.getMenu(row.data['actions'], this, ids);
        ...
        new Ext.dd.DropTarget(el, {
                    ...
                    listeners: {
                        success: {
                            fn: function () {
                                ...
                                if (grid.xtype == 'nastixstore-grid-products' && !grid.defaultNotify) {
                                        ...
                                        if (singleParent) {
                                            if (sourceNodes[0].data.parent != target.data.parent) {
                                                if (target.data.category_name == '') {
                                                    message = (sourceNodes.length > 1) ? _('nastixstore_drag_move_current_many_success') : _('nastixstore_drag_move_current_once_success');
                                                } else {
                                                    message = (sourceNodes.length > 1) ? String.format(_('nastixstore_drag_move_many_success'), target.data.category_name) : String.format(_('nastixstore_drag_move_one_success'), target.data.category_name);
                                                }
                                            }
                                            // else {
                                            //     message = (sourceNodes.length > 1) ? _('nastixstore_drag_sort_many_success') : _('nastixstore_drag_sort_once_success');
                                            // }
                                        } else {
                                            message = (sourceNodes.length > 1) ? String.format(_('nastixstore_drag_move_many_success'), target.data.category_name) : String.format(_('nastixstore_drag_move_one_success'), target.data.category_name);
                                        }
            ...
            _notifyMove: function (count, targetNode, dd) {
                ...
                if (targetNode.data.category_name == '') {
                    dd.ddel.innerHTML = (count > 1) ? _('nastixstore_drag_move_current_many') : _('nastixstore_drag_move_current_one');
                } else {
                    dd.ddel.innerHTML = (count > 1) ? String.format(_('nastixstore_drag_move_many'), targetNode.data.category_name) : String.format(_('nastixstore_drag_move_one'), targetNode.data.category_name);
                }
            },
            _notifySort: function (count, dd) {
                returnCls = 'x-tree-drop-ok-between';
                dd.ddel.innerHTML = (count > 1) ? _('nastixstore_drag_sort_many') : _('nastixstore_drag_sort_one');
            },
            _notifySelf: function (count, dd) {
                dd.ddel.innerHTML = (count > 1) ? _('nastixstore_drag_self_many') : _('nastixstore_drag_self_one');
            }
        });
    },
...
Ext.reg('nastixstore-grid-default', nastixstore.grid.Default);